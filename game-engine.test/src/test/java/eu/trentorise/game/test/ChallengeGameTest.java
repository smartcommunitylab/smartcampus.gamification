...
public class ChallengeGameTest extends GameTest {
	...
	public void initEnv() {
		...
		playerSrv.assignChallenge(GAME, PLAYER_1, "score", "specialInstance",
				data, today.toDate(), today.dayOfMonth().addToCopy(1).toDate());
		...
		data.put("bonusPointType", "green leaves");
		playerSrv.assignChallenge(GAME, PLAYER_2, "zeroImpact", null, data,
				today.toDate(), today.dayOfMonth().addToCopy(1).toDate());
	...
	public void defineGame() {
		...
		loadClasspathRules(
				GAME,
				Arrays.asList("rules/" + GAME + "/challenges.drl", "rules/"
						+ GAME + "/greenPoints.drl", "rules/" + GAME
						+ "/zeroimpactChallenge.drl"));
