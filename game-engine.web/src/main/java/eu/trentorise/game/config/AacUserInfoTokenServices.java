...
public class AacUserInfoTokenServices implements ResourceServerTokenServices {
	...
	private PlatformRolesClient platformRoleClient;
	...
	private OAuth2Authentication extractAuthentication(Map<String, Object> map) {
		...
		try {
			...
			List<String> roleDomains = platformRoleClient.getRolesByToken(accessToken);

			if (roleDomains.isEmpty()) {
				throw new SecurityException("user unauthorized.");
			}

			List<SimpleGrantedAuthority> authorities = roleDomains.stream()
					.map(domain -> new SimpleGrantedAuthority(domain)).collect(Collectors.toList());
	...
	public void setPlatformRoleClient(PlatformRolesClient platformRolesClient) {
		this.platformRoleClient = platformRolesClient;
