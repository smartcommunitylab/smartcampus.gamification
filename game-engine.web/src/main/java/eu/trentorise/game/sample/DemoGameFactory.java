	...
	public Game createGame(String gameId, String gameName, String gameOwner) {
		...
		if (gameFound) {
			...
			game.setConcepts(new HashSet<GameConcept>(
					Arrays.asList(new PointConcept("green leaves"), new PointConcept("health"), new PointConcept("p+r"),
							new BadgeCollectionConcept("green leaves"), new BadgeCollectionConcept("health"),
							new BadgeCollectionConcept("p+r"), new BadgeCollectionConcept("special"))));
			...
			GeneralClassificationTask task1 = new GeneralClassificationTask(finalClassSchedule, 3, "green leaves",
					"final classification green");
			game.getTasks().add(task1);

			GeneralClassificationTask task2 = new GeneralClassificationTask(finalClassSchedule, 3, "health",
					"final classification health");
			game.getTasks().add(task2);

			GeneralClassificationTask task3 = new GeneralClassificationTask(finalClassSchedule, 3, "p+r",
					"final classification p+r");
			game.getTasks().add(task3);

			// week classifications
			GeneralClassificationTask task4 = new GeneralClassificationTask(weekClassSchedule, 1, "green leaves",
					"week classification green");
			game.getTasks().add(task4);

			GeneralClassificationTask task5 = new GeneralClassificationTask(weekClassSchedule, 1, "health",
					"week classification health");
			game.getTasks().add(task5);

			GeneralClassificationTask task6 = new GeneralClassificationTask(weekClassSchedule, 1, "p+r",
			...
			try {
				...
				c = IOUtils.toString(Thread.currentThread().getContextClassLoader()
						.getResourceAsStream(DEMO_PACKAGE + "/" + DEMO_GAME + "/specialBadges.drl"));
				rule = new DBRule(gameId, c);
				rule.setName("specialBadges");
				gameSrv.addRule(rule);

				c = IOUtils.toString(Thread.currentThread().getContextClassLoader()
						.getResourceAsStream(DEMO_PACKAGE + "/" + DEMO_GAME + "/weekClassificationBadges.drl"));
				rule = new DBRule(gameId, c);
				rule.setName("weekClassificationBadges");
				gameSrv.addRule(rule);

				c = IOUtils.toString(Thread.currentThread().getContextClassLoader()
						.getResourceAsStream(DEMO_PACKAGE + "/" + DEMO_GAME + "/finalClassificationBadges.drl"));
				rule = new DBRule(gameId, c);
